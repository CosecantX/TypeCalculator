...
onready var spirit_label = preload("res://Labels/SpiritLabel.tscn")
...
func display_matchups():
	for type in types:
		if type != "NONE":
			...
			boxes[value].add_child(labels[type].instance())
...
func _on_TypeButton2_item_selected(index):
	run_program()
